...
// ─────────────────────────────────────────────────────────────────────────────
// Chick-fil-A x MyDojo Gift Certificate Campaign
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Gift certificate codes generated by admin for the Chick-fil-A promotion.
 * Each code represents a 2-week free trial.
 */
export const giftCertificates = mysqlTable("giftCertificates", {
  id: int("id").autoincrement().primaryKey(),
  /** Unique redemption code (e.g. CFA-ABCD-1234) */
  code: varchar("code", { length: 20 }).notNull().unique(),
  /** Campaign/batch name (e.g. "Chick-fil-A July 2026") */
  campaign: varchar("campaign", { length: 100 }).default("Chick-fil-A Promotion").notNull(),
  /** Expiry date for this certificate */
  expiresAt: timestamp("expiresAt"),
  /** Whether this code has been redeemed */
  redeemed: int("redeemed").default(0).notNull(), // 0 = not redeemed, 1 = redeemed
  ...
  childName: varchar("childName", { length: 200 }),
  /** Child's age if applicable */
  childAge: varchar("childAge", { length: 10 }),
  /** Whether the recipient requested a physical certificate mailed */
  mailRequested: int("mailRequested").default(0).notNull(),
  /** Mailing address if mail was requested */
  mailingAddress: text("mailingAddress"),
  /** Whether the follow-up SMS has been sent */
