    ...
    <div className="flex items-center justify-center gap-2 mb-8">
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} className={`h-2 rounded-full transition-all duration-300 ${
          i < current ? "bg-red-600 w-6" : i === current ? "bg-red-500 w-10" : "bg-gray-200 w-6"
        }`} />
      ))}
    </div>
  );
...
function RatingRow({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-3 py-3 border-b border-gray-100 last:border-0">
      <span className="flex-1 text-sm text-gray-700">{label}</span>
      <div className="flex gap-2">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} type="button" onClick={() => onChange(n)}
            className={`w-9 h-9 rounded-full text-sm font-bold transition-all ${
              value === n ? "bg-red-600 text-white shadow-md" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
            }`}>{n}</button>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="container mx-auto px-4 py-10 text-center">
          <div className="inline-flex items-center gap-2 bg-red-50 border border-red-200 text-red-600 text-xs font-bold uppercase tracking-widest px-4 py-2 rounded-full mb-5">
            <Award className="h-3 w-3" /> Saturday, August 15th, 2026
          </div>
          <h1 className="text-4xl md:text-5xl font-black uppercase mb-3 text-gray-900">
            Belt Test <span className="text-red-600">Intent to Promote</span>
          </h1>
          <p className="text-gray-500 max-w-xl mx-auto mb-8">
            Complete this form to declare your intent to test on <strong className="text-gray-900">Saturday, August 15th, 2026</strong>. The belt test fee of <span className="text-red-600 font-bold">$49.00 per student</span> is due at registration.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            {[
              ...
              <div key={label} className="bg-gray-50 border border-gray-200 rounded-xl px-6 py-4 text-center min-w-[160px]">
                <Icon className="h-5 w-5 text-red-500 mx-auto mb-2" />
                <div className="text-xs font-bold text-gray-800 uppercase tracking-wider">{label}</div>
      ...
      <div className="bg-red-50 border-b border-red-100 py-4">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1 text-sm text-gray-700">
            <div className="flex gap-2"><span className="text-red-500">&rsaquo;</span> Form must be submitted <strong className="text-gray-900">prior to August 10th</strong></div>
      ...
      <div className="container mx-auto px-4 py-12 max-w-2xl">
        ...
        {step === 0 && (
          <div>
            <h2 className="text-2xl font-black uppercase mb-2 text-center text-gray-900">Who is taking the belt test?</h2>
            <p className="text-gray-500 text-center text-sm mb-8">Select the option that best describes you</p>
            <div className="grid gap-4">
              {([
                ...
                <button key={value} type="button" onClick={() => setTestingFor(value)}
                  ...
                  <div className={`p-3 rounded-lg ${testingFor === value ? "bg-red-600 text-white" : "bg-gray-100 text-gray-600"}`}>
        ...
        {step === 1 && isAdult && (
          <div>
            <h2 className="text-2xl font-black uppercase mb-2 text-gray-900">Your Information</h2>
            <p className="text-gray-500 text-sm mb-6">Fill out all required fields below</p>
            <div className="grid gap-4">
              <div>
                <Label className="text-gray-700 mb-1 block">Your Full Name *</Label>
                <Input value={children[0].studentName} onChange={e => updateChild(0, { studentName: e.target.value })} placeholder="First and Last Name" className="bg-white border-gray-300 text-gray-900" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label className="text-gray-700 mb-1 block">Phone Number *</Label>
                  <Input value={phone} onChange={e => setPhone(e.target.value)} placeholder="(555) 555-5555" className="bg-white border-gray-300 text-gray-900" />
                </div>
                <div>
                  <Label className="text-gray-700 mb-1 block">Email Address *</Label>
                  <Input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="email@example.com" className="bg-white border-gray-300 text-gray-900" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label className="text-gray-700 mb-1 block">Current Belt *</Label>
                  <Select value={children[0].currentBelt} onValueChange={v => updateChild(0, { currentBelt: v })}>
                    <SelectTrigger className="bg-white border-gray-300 text-gray-900"><SelectValue placeholder="Select current belt" /></SelectTrigger>
                    <SelectContent className="bg-white border-gray-200 text-gray-900">{BELT_RANKS.map(b => <SelectItem key={b} value={b} className="text-gray-900 hover:bg-gray-100 focus:bg-gray-100">{b}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div>
                  <Label className="text-gray-700 mb-1 block">Belt Seeking *</Label>
                  <Select value={children[0].beltSeeking} onValueChange={v => updateChild(0, { beltSeeking: v })}>
                    <SelectTrigger className="bg-white border-gray-300 text-gray-900"><SelectValue placeholder="Select belt seeking" /></SelectTrigger>
                    <SelectContent className="bg-white border-gray-200 text-gray-900">{BELT_RANKS.map(b => <SelectItem key={b} value={b} className="text-gray-900 hover:bg-gray-100 focus:bg-gray-100">{b}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
              </div>
              <div>
                <Label className="text-gray-700 mb-1 block">Instructor</Label>
                <Select value={children[0].instructor} onValueChange={v => updateChild(0, { instructor: v })}>
                  <SelectTrigger className="bg-white border-gray-300 text-gray-900"><SelectValue placeholder="Select instructor" /></SelectTrigger>
                  <SelectContent className="bg-white border-gray-200 text-gray-900">{INSTRUCTORS.map(i => <SelectItem key={i} value={i} className="text-gray-900 hover:bg-gray-100 focus:bg-gray-100">{i}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
          </div>
        ...
        {step === 1 && !isAdult && (
          <div>
            <h2 className="text-2xl font-black uppercase mb-2 text-gray-900">{isMyselfAndChild ? "Your Information" : "Parent / Guardian Information"}</h2>
            <p className="text-gray-500 text-sm mb-6">{isMyselfAndChild ? "Your contact info — you will be added as a student along with your children" : "Your contact information (shared for all children)"}</p>
            {/* Smart lookup */}
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
              <Label className="text-gray-700 mb-2 block text-sm font-semibold flex items-center gap-2"><Search className="h-4 w-4 text-blue-500" /> Quick Lookup (Optional)</Label>
                  ...
                  {lookupResults.data.map((s) => (
                    <button key={s.id} type="button" onClick={() => applyLookup(s)}
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-xs">{s.phone} {s.program ? `· ${s.program}` : ""}</p>
                      </div>
            ...
            <div className="grid gap-4">
              <div>
                <Label className="text-gray-700 mb-1 block">Parent / Guardian Name *</Label>
                <Input value={parentName} onChange={e => setParentName(e.target.value)} placeholder="Your full name" className="bg-white border-gray-300 text-gray-900" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label className="text-gray-700 mb-1 block">Phone Number *</Label>
        ...
        {step === 2 && !isAdult && (
          <div>
            <h2 className="text-2xl font-black uppercase mb-2 text-gray-900">Student(s) Information</h2>
            <p className="text-gray-500 text-sm mb-6">{isMyselfAndChild ? "Add yourself and each child testing. You can register multiple students at once." : "Add each child testing. You can register multiple children at once."}</p>
            <div className="grid gap-4">
              {children.map((child, idx) => (
                <div key={idx} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-bold text-red-600 uppercase tracking-wider text-sm">
                      {isMyselfAndChild && idx === 0 ? "Myself (Parent/Adult)" : children.length > 1 ? `Child ${idx + 1}` : "Student"}
                    </h3>
                    {children.length > 1 && (
                      <button type="button" onClick={() => removeChild(idx)} className="text-gray-400 hover:text-red-500 transition-colors">
                  ...
                  <div className="grid gap-3">
                    <div>
                      <Label className="text-gray-700 mb-1 block text-sm">Student Name *</Label>
                      <Input value={child.studentName} onChange={e => updateChild(idx, { studentName: e.target.value })} placeholder="Student's full name" className="bg-gray-50 border-gray-300 text-gray-900" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <Label className="text-gray-700 mb-1 block text-sm">Current Belt *</Label>
                        <Select value={child.currentBelt} onValueChange={v => updateChild(idx, { currentBelt: v })}>
                          <SelectTrigger className="bg-gray-50 border-gray-300 text-gray-900"><SelectValue placeholder="Current belt" /></SelectTrigger>
                          <SelectContent className="bg-white border-gray-200 text-gray-900">{BELT_RANKS.map(b => <SelectItem key={b} value={b} className="text-gray-900 hover:bg-gray-100 focus:bg-gray-100">{b}</SelectItem>)}</SelectContent>
                        </Select>
                      </div>
                      <div>
                        <Label className="text-gray-700 mb-1 block text-sm">Belt Seeking *</Label>
                        <Select value={child.beltSeeking} onValueChange={v => updateChild(idx, { beltSeeking: v })}>
                          <SelectTrigger className="bg-gray-50 border-gray-300 text-gray-900"><SelectValue placeholder="Belt seeking" /></SelectTrigger>
                          <SelectContent className="bg-white border-gray-200 text-gray-900">{BELT_RANKS.map(b => <SelectItem key={b} value={b} className="text-gray-900 hover:bg-gray-100 focus:bg-gray-100">{b}</SelectItem>)}</SelectContent>
                        </Select>
                      </div>
                    </div>
                    <div>
                      <Label className="text-gray-700 mb-1 block text-sm">Instructor</Label>
                      <Select value={child.instructor} onValueChange={v => updateChild(idx, { instructor: v })}>
                        <SelectTrigger className="bg-gray-50 border-gray-300 text-gray-900"><SelectValue placeholder="Select instructor" /></SelectTrigger>
                        <SelectContent className="bg-white border-gray-200 text-gray-900">{INSTRUCTORS.map(i => <SelectItem key={i} value={i} className="text-gray-900 hover:bg-gray-100 focus:bg-gray-100">{i}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                  </div>
              ...
              <button type="button" onClick={addChild}
                className="flex items-center justify-center gap-2 border-2 border-dashed border-gray-300 hover:border-red-400 text-gray-500 hover:text-red-500 rounded-xl py-4 transition-all bg-white">
                <Plus className="h-5 w-5" /> {isMyselfAndChild ? "Add Another Student" : "Add Another Child"}
              </button>
            </div>
            {children.length > 1 && (
              <div className="mt-4 bg-white border border-gray-200 rounded-lg px-4 py-3 text-sm text-gray-700 shadow-sm">
                <span className="text-gray-900 font-bold">{children.length} student{children.length > 1 ? "s" : ""}</span> — Total: <span className="text-red-600 font-bold">${children.length * 49}.00</span> ($49 each)
              </div>
            )}
          </div>
        )}
                ...
                {children.map((c, i) => (
                  <button key={i} type="button" onClick={() => setActiveChildIdx(i)}
                    className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all ${
                      activeChildIdx === i ? "bg-red-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}>{c.studentName || `Child ${i + 1}`}</button>
                ))}
              </div>
            )}
            <h2 className="text-2xl font-black uppercase mb-1 text-gray-900">
              Section 1: Self-Reflection
              {!isAdult && <span className="text-red-600"> — {children[activeChildIdx].studentName || `Child ${activeChildIdx + 1}`}</span>}
            </h2>
            <p className="text-gray-500 text-sm mb-2">Rate from <strong className="text-gray-900">1 (Needs Work)</strong> to <strong className="text-gray-900">5 (Excellent)</strong></p>
            <div className="flex gap-6 text-xs text-gray-400 mb-4"><span>1 = Needs Work</span><span>3 = Average</span><span>5 = Excellent</span></div>
        ...
        {((isAdult && step === 3) || (!isAdult && step === 4)) && (
          <div>
            ...
            <p className="text-gray-500 text-sm mb-6">Please answer each question thoughtfully and honestly.</p>
            <div className="grid gap-6">
              {WRITTEN_QUESTIONS.map((q, i) => {
                const idx = isAdult ? 0 : activeChildIdx;
                return (
                  <div key={i}>
                    <Label className="text-gray-800 mb-2 block text-sm font-semibold">{i + 1}. {q}</Label>
                    <Textarea value={children[idx].writtenAnswers[i]}
                      ...
                      placeholder="Your answer..." className="bg-white border-gray-300 text-gray-900 min-h-[80px]" />
                  </div>
                );
              })}
            </div>
        ...
        {!isAdult && step === 5 && (
          <div>
            <h2 className="text-2xl font-black uppercase mb-1 text-gray-900">Section 3: Parent Evaluation</h2>
            <p className="text-gray-500 text-sm mb-4">Rate 1&ndash;5 for each item. My child&hellip;</p>
            ...
            <div className="grid gap-5 mb-6">
              {PARENT_QUESTIONS.map((q, i) => (
                <div key={i}>
                  <Label className="text-gray-800 mb-2 block text-sm font-semibold">{q}</Label>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
              <Label className="text-gray-800 mb-3 block font-semibold">Would you recommend your child for promotion?</Label>
                ...
                {[{ value: "yes", label: "✓ Yes" }, { value: "not_yet", label: "✗ Not Yet" }].map(opt => (
                  <button key={opt.value} type="button"
                    ...
                    className={`flex-1 py-3 rounded-lg font-bold text-sm transition-all ${
                      parentRecommendation === opt.value
                        ? "bg-red-600 text-white"
                        : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}>{opt.label}</button>
                ))}
              </div>
              <Label className="text-gray-600 mb-2 block text-sm">Comments (optional)</Label>
              <Textarea value={parentComments} onChange={e => setParentComments(e.target.value)}
                placeholder="Any additional comments..." className="bg-gray-50 border-gray-300 text-gray-900" />
            </div>
          </div>
        )}
        ...
        {((isAdult && step === 4) || (!isAdult && step === 6)) && (
          <div>
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl font-black uppercase mb-2 text-gray-900">The Black Belt Question</h2>
              <p className="text-gray-500 text-sm max-w-md mx-auto">This question reminds us that the journey is bigger than the belt.</p>
            </div>
            <div className="grid gap-6">
              {children.map((child, idx) => (
                <div key={idx} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  {children.length > 1 && <h3 className="font-bold text-red-600 text-sm uppercase mb-3">{child.studentName || `Child ${idx + 1}`}</h3>}
                  <p className="text-base font-semibold text-gray-900 mb-4 leading-relaxed">
                    &ldquo;If you earn this belt, how will you become a <span className="text-red-600">better person</span>, not just a better martial artist?&rdquo;
                  </p>
                  <Textarea value={child.blackBeltAnswer} onChange={e => updateChild(idx, { blackBeltAnswer: e.target.value })}
                    placeholder="Share your thoughts here..." className="bg-gray-50 border-gray-300 text-gray-900 min-h-[120px]" />
                </div>
              ))}
            </div>
          </div>
        ...
        {((isAdult && step === 5) || (!isAdult && step === 7)) && (
          <div>
            <h2 className="text-2xl font-black uppercase mb-6 text-center text-gray-900">Pledge &amp; Payment</h2>
            {children.map((child, idx) => (
              <div key={idx} className="bg-white border border-gray-200 rounded-xl p-5 mb-4 shadow-sm">
                <h3 className="font-bold text-red-600 uppercase tracking-wider text-sm mb-3">
                ...
                <div className="flex items-start gap-3">
                  <Checkbox id={`pledge-${idx}`} checked={child.studentPledge} onCheckedChange={v => updateChild(idx, { studentPledge: v === true })} className="mt-0.5 border-gray-400" />
                  <label htmlFor={`pledge-${idx}`} className="text-sm text-gray-700 cursor-pointer">
            ...
            {!isAdult && (
              <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4 shadow-sm">
                <h3 className="font-bold text-red-600 uppercase tracking-wider text-sm mb-3">Parent Commitment</h3>
                <p className="text-gray-600 text-sm leading-relaxed mb-4 italic">
                ...
                <div className="flex items-start gap-3">
                  <Checkbox id="parentCommitment" checked={parentCommitment} onCheckedChange={v => setParentCommitment(v === true)} className="mt-0.5 border-gray-400" />
                  <label htmlFor="parentCommitment" className="text-sm text-gray-700 cursor-pointer">I agree to the Parent Commitment</label>
                </div>
              </div>
            )}
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6 shadow-sm">
              {children.map((child, idx) => (
                <div key={idx} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
                  <span className="text-gray-600 text-sm">Belt Test Fee — {child.studentName || `Child ${idx + 1}`}</span>
        ...
        <div className="flex gap-3 mt-8">
          {step > 0 && (
            <Button type="button" variant="outline" onClick={back} className="flex-1 border-gray-300 text-gray-700 hover:bg-gray-100 bg-white">
        ...
        <p className="text-center text-xs text-gray-400 mt-6">
          Questions? Call us at <a href="tel:8774693656" className="text-red-600 hover:underline">(877) 4-MYDOJO</a> or visit the front desk.
        </p>
      </div>
    </div>
  );
