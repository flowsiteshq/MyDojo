...
function resolveNextStep(state: IntakeState): IntakeStep {
  ...
  // 2b. Adult "other" enrollee name (stored in childName field for reuse)
  // Skip age entirely for adult "other" enrollees — they go straight to emotional discovery
  if (state.classFor === "other" && !state.childName && !state.completedSteps.includes(IntakeStep.CHILD_NAME)) {
    return IntakeStep.CHILD_NAME;
  }

  // 3. Child's age (determine program/segment) — ONLY for child flow, never for "other"
  if (state.classFor === "child" && !state.childAge && !state.completedSteps.includes(IntakeStep.CHILD_AGE)) {
    return IntakeStep.CHILD_AGE;
  }
  
  // 4. Rapid rapport for adults (prior training?) — for "self" and "other" adult enrollees
  if ((state.classFor === "self" || state.classFor === "other") && state.priorTraining === null && !state.completedSteps.includes(IntakeStep.RAPID_RAPPORT_ADULT)) {
  ...
  switch (currentStep) {
    ...
    case IntakeStep.CLASS_FOR: {
      ...
      if (parsedClassFor) {
        ...
        if (nextStep === IntakeStep.CHILD_NAME) {
          // Use context-aware question: adult enrollee vs child
          const nameQuestion = state.classFor === "child"
            ? "Great! What's your child's name?"
            : "Great! What's their name?";
          const response = {
            assistantMessage: nameQuestion,
            state,
            collectedFields: getCollectedFields(state),
    ...
    case IntakeStep.CHILD_NAME: {
      ...
      if (childNameResult.valid && childNameResult.name) {
        ...
        const response = {
          assistantMessage: childNameResult.error || (state.classFor === "child" ? "What's your child's name?" : "What's their name?"),
          state,
          collectedFields: getCollectedFields(state),
          nextExpectedField: IntakeStep.CHILD_NAME,
          error: null,
      ...
      if (state.priorTraining === null) {
        if (normalizedInput && normalizedInput.length > 0) {
          ...
          if (hasPriorTraining) {
            ...
            // Default to adult program for self/other flows (no age collected)
            if (!state.segment || !state.program) {
              state.segment = Segment.ADULTS;
              state.program = SEGMENT_TO_PROGRAM[Segment.ADULTS];
            }
    ...
    case IntakeStep.EMOTIONAL_DISCOVERY: {
      ...
      if (!state.emotionalGoal) {
        ...
        if (normalizedInput && normalizedInput.length > 2) {
          ...
          const response = {
            assistantMessage: state.classFor === "child"
              ? `That's exactly what we help families achieve! If ${state.childName || "they"} developed that, what would be different at home or school?`
              : state.classFor === "other"
              ? `That's exactly what we help with! If ${state.childName || "they"} developed that, what would be different for them at home or work?`
              : `That's exactly what we help with! If you developed that, what would be different for you at home or work?`,
            state,
            collectedFields: getCollectedFields(state),
          ...
          const who = state.classFor === "child"
            ...
            : state.classFor === "other"
            ? (state.childName || "them")  // childName holds the enrollee's name for "other" flow
            : "yourself";
          const response = {
        ...
        if (normalizedInput && normalizedInput.length > 2) {
          ...
          const response = {
            assistantMessage: state.classFor === "child"
              ? `If ${state.childName || "they"} developed that, what would be different at home or school?`
              : state.classFor === "other"
              ? `If ${state.childName || "they"} developed that, what would be different for them?`
              : `If you developed that, what would be different for you?`,
            state,
            collectedFields: getCollectedFields(state),
            nextExpectedField: IntakeStep.EMOTIONAL_DISCOVERY,
            error: null,
    ...
    case IntakeStep.TRIAL_TRANSITION: {
      ...
      // Generate default slots using CST/CDT (America/Chicago) so labels are correct
      ...
      const slot1: Slot = {
        id: "default-1",
        day: "Today",
        date: nowCST.toLocaleDateString("en-CA"),
        time: "6:15 PM",
      ...
      const slot2: Slot = {
        id: "default-2",
        day: "Tomorrow",
        date: tomorrowCST.toLocaleDateString("en-CA"),
        time: "5:30 PM",
      ...
      const isChildFlow = state.classFor === "child";
      const transitionMessage = isChildFlow
        ? "That's exactly why most families start with our 2-Week Trial. " +
        ...
        : "That's exactly why most people start with our 2-Week Trial. " +
          "It gives you full classes plus the official uniform so you can experience everything properly. " +
          "We're enrolling new students this week and spots fill quickly.\n\n";
    ...
    case IntakeStep.FAST_FIRST_LESSON: {
      ...
      if (!state.selectedSlot) {
        ...
        if (state.offeredSlots.length === 0) {
          // Generate default slots in CST/CDT (America/Chicago)
          const nowCST2 = new Date(new Date().toLocaleString("en-US", { timeZone: "America/Chicago" }));
          const tomorrowCST2 = new Date(nowCST2);
          tomorrowCST2.setDate(tomorrowCST2.getDate() + 1);
          
          const slot1: Slot = {
            id: "default-1",
            day: "Today",
            date: nowCST2.toLocaleDateString("en-CA"),
            time: "6:15 PM",
            displayText: "Today at 6:15 PM"
          };
          ...
          const slot2: Slot = {
            id: "default-2",
            day: "Tomorrow",
            date: tomorrowCST2.toLocaleDateString("en-CA"),
            time: "5:30 PM",
            displayText: "Tomorrow at 5:30 PM"
          };
          ...
          // Track how many times user has rejected slots
          const neitherCount = state.offeredSlots.length / 2; // 1 = first rejection, 2 = second rejection

          if (input.includes("neither") || input.includes("don't work") || input.includes("doesn't work") || input.includes("other") || input.includes("still") || input.includes("no")) {
            // If user has already rejected once (offeredSlots has 2 slots from first offer), offer 2 more
            // If user has rejected twice (offeredSlots has 4 slots), ask for their availability
            if (neitherCount >= 2) {
              // User has rejected slots twice — stop looping, ask for their availability
              const response = {
                assistantMessage: "No worries at all! What days and times typically work best for you? We'll find something that fits your schedule.",
                state,
                collectedFields: getCollectedFields(state),
                nextExpectedField: IntakeStep.FAST_FIRST_LESSON,
                quickReplies: ["Weekday mornings", "Weekday evenings", "Weekends", "Call me to schedule"],
                error: null,
              };
              addToMessageHistory(state, "assistant", response.assistantMessage);
              return response;
            ...
            // First rejection — generate 2 new slots (day after tomorrow)
            const nowCST3 = new Date(new Date().toLocaleString("en-US", { timeZone: "America/Chicago" }));
            const dayAfterTomorrow = new Date(nowCST3);
            dayAfterTomorrow.setDate(dayAfterTomorrow.getDate() + 2);
            const datStr = dayAfterTomorrow.toLocaleDateString('en-US', { weekday: 'long' });
            
            const slot3: Slot = {
              id: "default-3",
              day: datStr,
              date: dayAfterTomorrow.toLocaleDateString("en-CA"),
              time: "6:00 PM",
              displayText: `${datStr} at 6:00 PM`
            };
            ...
            const slot4: Slot = {
              ...
              date: dayAfterTomorrow.toLocaleDateString("en-CA"),
              time: "7:00 PM",
              displayText: `${datStr} at 7:00 PM`
            };
            
            state.offeredSlots = [...state.offeredSlots, slot3, slot4];
            ...
            const response = {
              ...
              collectedFields: getCollectedFields(state),
              nextExpectedField: IntakeStep.FAST_FIRST_LESSON,
              quickReplies: [slot3.displayText, slot4.displayText, "Neither works"],
              error: null,
            };
            addToMessageHistory(state, "assistant", response.assistantMessage);
            return response;
    ...
    case IntakeStep.ENROLLMENT_PREFRAME: {
      ...
      const isChildEnrollment = state.classFor === "child";
      const message = isChildEnrollment
        ? "After the first lesson, most families know right away this is the perfect fit. " +
        ...
        : state.classFor === "other"
        ? `After the first lesson, most people know right away this is the perfect fit. ` +
          `If ${state.childName || "they"} love it as much as I think they will, I'll show you the membership options and how to save by enrolling that day. ` +
          `The first class lasts about 50 minutes — they'll get a full taste of our training style.`
        : "After the first lesson, most people know right away this is the perfect fit. " +
          "If you love it as much as I think you will, I'll show you the membership options and how to save by enrolling that day. " +
          "The first class lasts about 50 minutes — you'll get a full taste of our training style.";
    ...
    case IntakeStep.CONFIRM: {
      ...
      const contactLabel = state.classFor === "child" ? "Parent" : "Contact";
      const studentLabel = state.classFor === "child" ? "👶 Student" : "🥋 Student";
      const studentName = state.classFor === "child"
        ? (state.childName || "Your child")
        : state.classFor === "other"
        ? (state.childName || "Your guest")
        : state.name;
      const summaryMessage = `📋 **Booking Summary**\n\n` +
        `👤 ${contactLabel}: ${state.name}\n` +
        `📞 Phone: ${state.phone}\n` +
        `📧 Email: ${state.email || "Not provided"}\n` +
        `${studentLabel}: ${studentName}${ageInfo}\n` +
